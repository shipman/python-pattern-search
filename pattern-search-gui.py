...
class Ui_Dialog_First_Window(object):
    def setupUi(self, Dialog):
        ...
        self.cb_label = QtWidgets.QLabel(Dialog)
        self.cb_label.setObjectName("cb_label")
        self.gridLayout.addWidget(self.cb_label, 3, 0, 1, 2)
        self.cb = QtWidgets.QComboBox(Dialog)
        self.cb.setObjectName("cb") # Do we have to add a function to deal with it if someone changes the value? Or will it just work?
        self.cb.setToolTip("This is the function to be optimized.")
        self.cb.addItems(["Spherical", "Ackley", "Beale", "Rastrigin", "Easom"])
        self.gridLayout.addWidget(self.cb, 3, 2, 1, 1)
        self.num_dim_label = QtWidgets.QLabel(Dialog)
        self.num_dim_label.setObjectName("num_dim_label")
        self.gridLayout.addWidget(self.num_dim_label, 4, 0, 1, 2)
        ...
        self.gridLayout.addWidget(self.num_dim_input, 4, 2, 1, 1)
        self.bound_label = QtWidgets.QLabel(Dialog)
        self.bound_label.setObjectName("bound_label")
        self.gridLayout.addWidget(self.bound_label, 5, 0, 1, 2)
        ...
        self.gridLayout.addWidget(self.bound_cb, 5, 2, 1, 1)
        ...
        self.gridLayout.addWidget(self.next_steps_button, 6, 0, 1, 1)
        ...
        self.gridLayout.addWidget(self.run_search_button, 6, 1, 1, 2)
        self.exit_button = QtWidgets.QPushButton(Dialog)
        self.exit_button.setObjectName("exit_button")
        self.exit_button.clicked.connect(app.quit)
        self.gridLayout.addWidget(self.exit_button, 7, 0, 1, 3)
    ...
    def retranslateUi(self, Dialog):
        ...
        self.cb_label.setText(_translate("Dialog", "Function to Optimize"))
...
class Ui_Dialog_CoordInit_NoBounds(QtWidgets.QDialog):
    def setupUi(self, Dialog3a, num_dimensions):
        ...
        Dialog3a.setWindowTitle(_translate("Dialog3a", "Coordinate Initialization"))
...
class PlotCanvas(FigureCanvas): # Taken from http://littlecaptain.net/pyqt5-matplotlib.html
    def __init__(self, xdata, ydata, distdata, parent=None, width=8, height=6, dpi=100):
        fig = Figure(figsize=(width, height), dpi=dpi)
        FigureCanvas.__init__(self,fig)
        self.setWindowTitle("Plot of Optimization Results")

        self.axes = fig.add_subplot(121)
        self.axes.plot(xdata,ydata,'o')
        self.axes.set_xlabel('Iteration #')
        self.axes.set_ylabel('Value of Function')
        self.axes.xaxis.set_major_locator(MaxNLocator(nbins=5,integer=True))

        self.axes2 = fig.add_subplot(122)
        self.axes2.plot(xdata,distdata,'o')
        self.axes2.set_xlabel('Iteration #')
        self.axes2.set_ylabel('Distance from Starting Point')
        self.axes2.xaxis.set_major_locator(MaxNLocator(nbins=5,integer=True))

        #fig.subplots_adjust(wspace=0.2)
        fig.suptitle('Pattern Search Optimization Results')
        fig.tight_layout(rect=[0, 0, 1, 0.95])

        self.show()

def optimization_function(point,choice): # Need to make sure that functions match with number of dimensions; not really an obvious way to do that, probably should just have that be a specification of each test function.
# Spherical function
    if choice == "Spherical":
        x = point[0]
        y = point[1]
...
def test_points(point_list,choice):
    output_value_list = []

    for point in point_list:
        temp_output = optimization_function(point,choice)
...
def report_init_coords(data_object):
    ...
    if data_object.bounds_flag == True:
        data_object.exit_button.setEnabled(True)
...
def pattern_search(data_object):
    ...
    while convergence_flag == False:
        ...
        output_values = test_points(trial_points,choice)
        ...
        for i in range(num_dimensions):
            temp_dist += (best_point[i] - orig_point[i])**2

        temp_dist = math.sqrt(temp_dist)
        dist_data.append(temp_dist)

        convergence_flag = convergence_check(num_iterations,max_iterations,step_size,conv_ss)

    print "Finished!\n"

    m = PlotCanvas(x_data,y_data,dist_data,width=5,height=4,dpi=100)



    data_object.run_search_button.setEnabled(False) # Grey out button so user has to set up a new run (maybe remove if this gets annoying?)
